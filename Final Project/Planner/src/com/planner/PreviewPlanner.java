...
public class PreviewPlanner {
	...
	private static final float PERCENT_SAFE_BOUND = 0.15f;
	private static final float PERCENT_SAFE_DISCOUNT = 0.2f;
    private static final float TURN_SAFE_HALF_RANGE = 50.0f;
	...
	private State Propagate(State state, int action, EntityData ship, ArrayList<EntityData> asteroids, 
		...
		PropagateShip(ship, asteroids, state.exclusions, target_h, state.met);
...
//	}
	...
	public PreviewPlanner(int h, float d) {
        ...
        planArr = new ArrayList[Decision.NDECISIONS];

        for (int i = 0; i < Decision.NDECISIONS; i++)
            planArr[i] = new ArrayList<PlanAction>();
	}
	...
	private int ComputeDiscountedRewards(EntityData ship, ArrayList<EntityData> asteroids) {
		...
		// We are in very bad situaton, do nothing
		if (state == null)
			return -1;
	...
	private void ComputeDiscountedRewardsWorker(int layer, State state, EntityData ship, 
		...
		edgeRewards[index] = reward * (float)Math.pow(discount,layer-1) + TURNPENALTY * state.met.getTurnAngleAbs();	// discounted reward
	...
	public ArrayList<PlanAction> determinePlan(EntityData ship, ArrayList<EntityData> asteroids, GameData gd) {

		ArrayList<PlanAction> plan = null;
        m_GameData = gd;
		int action;
		...
		try {
            ...
            if (horizon > 0) {
                ...
                long planDiff = System.currentTimeMillis() - startTime;
                plan = planArr[action];

                for (PlanAction act : plan) {
                    act.shiftTime(planDiff);
                }

            // If not, just use the closest heading
            } else {
    ...
    private DeltaVCurve CalculateAsteriodStateMetricsDeltaVCurve(EntityData ship, ArrayList<EntityData> asteroids, Metrics met) {
        ...
        for (EntityData asteroid : asteroids)
        {
            ...
            met.addAsteroid(distance, angle, impactTime, HEADING_RANGE);
        }
		...
		for (EntityData asteroid : asteroids)
		{
			...
			if (relpos.len() - config_radius < SAFE_DISTANCE)
			{
				...
				for (int ndx = 0; ndx < occ_intersect.size(); ndx++)
				{
					...
					dv.rotate(ang);
					dv.scl(Ship.SHIP_LINEAR_ACCELERATION * (deltaT - (float) Math.toRadians(ang < 180
					                                                                         ? ang
...
//		System.out.println(metrics.getPercent_safe());

		return exclusions;
	}


//	private float selectHeading(EntityData ship, ArrayList<EntityData> asteroids, ExclusionZones exclusions)
//	{
...
//	}
	...
	private void PropagateShip(EntityData ship, ArrayList<EntityData> asteroids, ExclusionZones exclusions, float target_h, Metrics metrics)
	{
        float deltaT = metrics.getDeltaT();
...
//        newV.rotate(ship.getAngle() + target_h);
	...
	private void calculatePlan(ArrayList<PlanAction> plan, EntityData ship, ArrayList<EntityData> asteroids, ExclusionZones exclusions, float target_h, long start_time, Metrics metrics, float reward)
	{
        float deltaT = metrics.getDeltaT();
		float turn_angle = (target_h <= 180.0f) ? (target_h) : (target_h-360);
		float turn_time = (float) Math.toRadians(Math.abs(turn_angle)) / Ship.SHIP_ANGULAR_VELOCITY;
...
//        if (Math.abs(turn_angle) < SHOOTBOUND && reward > 0)
//            plan.add(new PlanAction(start_time, PlanAction.Action.fire));
		...
		} else
		{
			...
			plan.add(new PlanAction(start_time + (long) (deltaT * 1000), PlanAction.Action.stopForward));
		}

//		Vector2 newV = new Vector2(1.0f, 0.0f);
//		newV.rotate(ship.getAngle() + turn_angle);
//		newV.scl(burntime * Ship.SHIP_LINEAR_ACCELERATION);r
//		if (newV.len() > Ship.SHIP_MAX_LINEAR_VELOCITY)
//		{
//			newV.nor();
//			newV.scl(Ship.SHIP_MAX_LINEAR_VELOCITY);
